...
/**
 * @notice A proof-of-concept implementation demonstrating how Flexible Voting can be used to allow
 * holders of
 * governance tokens to use them in DeFi but still participate in governance. The FractionalPool
 * simulates a
...
contract FractionalPool {
  ...
  function withdraw(uint256 _amount) public {
    deposits[msg.sender] -= _amount; // overflows & reverts if user does not have sufficient
      // deposits
   ...
   * holds.
   * Uses the sum of all depositor voting expressions to decide how to split its voting weight. Can
   * be
   * called by anyone, but _must_ be called within `CAST_VOTE_WINDOW` blocks before the proposal
   * deadline.
  ...
  /**
   * @notice Method which returns the deadline for depositors to express their voting preferences to
   * this Pool
  ...
  function getPastDeposits(address account, uint256 blockNumber)
    public
    view
    virtual
    returns (uint256)
